import {
  ...
  ColumnsNode, GroupByFrameNode, PivotNode, UnpivotNode, NestNode, UnnestNode, AppendNode, BindColumnsNode, RenameNode, SplitColumnNode, AddIndexNode, DecisionMatrixNode, DecisionSensitivityNode, AllocatorNode, SettleNode, PayoffPlannerNode, ScheduleNode,
  ReconcileNode,
    ...
    children: [
      ...
      },
      {
        ...
        children: [
          ...
          { type: "write-properties", label: "Write Properties", description: "Writes a cube of rows back into notes' frontmatter, keyed by a path column. Each column becomes a property, written in the form its type asks for: dates unquoted, lists as blocks, note names as links. Preview reads the notes and shows what each write would add, change, or leave alone; Run patches the YAML line by line, never rewriting the rest of the note. Loads disarmed, and only the Run button writes. Desktop only.", create: () => new WritePropertiesNode(), parity: false, keywords: "write properties obsidian frontmatter yaml note cube sink update patch bases plan preview" },
        ],
      },
    ],
  },
    ...
    children: [
        ...
        children: [
          ...
          },
          {
            ...
            children: [
              ...
              { type: "schedule", label: "Schedule", description: "Works out when each task in a project starts and finishes from its duration and what it waits on: the critical-path method. Rows are tasks; Task is the first text column, Duration the first number column in days (blank or 0 is a milestone), and Predecessors a list cell naming the tasks that must finish first. Working days skips weekends and the Holidays list; Calendar days counts every day. Output: the rows with Start, Finish, Float and Critical appended, where Float is how many days a task can slip without moving the finish and Critical marks the tasks with none. Project finish is the last finish, and Gantt is Mermaid source a Mermaid node draws, with one section per Project column value.", create: () => new ScheduleNode(), parity: false, keywords: "schedule cpm critical path gantt project plan timeline tasks predecessors dependencies float slack milestone duration working days finish date pert" },
              { type: "payoff-planner", label: "Payoff Planner", description: "Plans paying off several debts: rows carry each Balance, APR and minimum payment, plus an extra amount you pay every month. Every minimum is paid; the extra and each freed minimum go to the head debt, avalanche (highest APR first, the least interest) or snowball (smallest balance first, the quickest win). Summary gives months, interest and payoff date per debt; Schedule gives each month's balances. Closed-form month by month, no solver.", create: () => new PayoffPlannerNode(), parity: false, keywords: "debt payoff planner avalanche snowball loans credit card interest extra payment months schedule" },
              { type: "settle",      label: "Group Cost Settle", description: "Squares up a group that paid unevenly: rows are people with what each Paid, and the result is who pays whom in the fewest transfers (From · To · Amount) plus each person's share and net. Equal split by default; By Share weighs what each owes by a Share column (a couple as 2, a child as 0.5). Biggest creditor takes from biggest debtor, exact to the cent.", create: () => new SettleNode(), parity: false, keywords: "settle settlement split bill trip expenses who owes whom debts transfers splitwise group cost share even up" },
      ...
      },
      {
        ...
        children: [
          ...
          { type: "cube-input", label: "Cube Input", description: "Type a Cube directly: rows of records where a value can be a number, text, a list, or a nested table of records. The literal source beside Table, Frame and List Input, edited in the same popup: open a cell to edit its list or nested table in place.", create: () => new CubeInputNode(), parity: false, keywords: "cube input literal type records nested list json source" },
