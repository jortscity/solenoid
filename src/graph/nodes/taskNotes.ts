...
export class TaskNotesNode extends ClassicPreset.Node {
  ...
  private async fetchProvider(from: number, to: number): Promise<void> {
    ...
    try {
      ...
      const friendly = /HTTP 401/.test(msg) ? "Token rejected. Paste the plugin's API token." : /Failed to fetch|ECONNREFUSED|error sending request|NetworkError/i.test(msg) ? "Can't reach TaskNotes. Turn on its HTTP API and check the port in Settings." : msg;
